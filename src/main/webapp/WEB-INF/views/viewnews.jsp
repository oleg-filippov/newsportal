...
<c:set var="title">${news.title} | <spring:message
		code="project.title" />
</c:set>
...
<sec:authorize access="hasRole('ROLE_AUTHOR')">
	<c:if test="${loggedUserId == news.author.id}">
...
<sec:authorize access="hasRole('ROLE_AUTHOR')">
	<c:if test="${isThisNewsAuthor || isAdmin}">
		<a href="${editNewsUrl}" class="btn btn-info btn-xs" type="button">
			<i class="glyphicon glyphicon-edit"></i> <spring:message
						code="news.editNewsUrl" />
				</a>
		<a href="${deleteNewsUrl}" class="btn btn-danger btn-xs" type="button"
					onclick="return deleteConfirm();"><i
					class="glyphicon glyphicon-exclamation-sign">
				</i> <spring:message code="news.deleteNewsUrl" /></a>
				<br>
	</c:if>
</sec:authorize>
<hr>
...
<sec:authorize access="isAnonymous()">
	<c:set var="signin">
		<a href="${signinUrl}"><spring:message
						code="viewnews.loginComments.signin" /></a>
	</c:set>
	<c:set var="signup">
		<a href="${signupUrl}"><spring:message
						code="viewnews.loginComments.signup" /></a>
	</c:set>
	<spring:message code="viewnews.loginComments"
				arguments="${signin},${signup}" />
</sec:authorize>
...
<sec:authorize access="hasRole('ROLE_USER')">
	<form:form action="${addCommentUrl}" method="post"
		...
		<h4>
					<form:errors path="content" class="label label-danger" />
				</h4>
		<spring:bind path="content">
			<textarea class="form-control" name="content" maxlength="500"
						rows="3"
						placeholder="<spring:message code="viewnews.leaveComment" />"></textarea>
		</spring:bind>
		<button class="btn" type="submit">
					<spring:message code="viewnews.addCommentButton" />
				</button>
  		<button class="btn" type="reset">
					<spring:message code="viewnews.resetCommentButton" />
				</button>
	</form:form>
</sec:authorize>
...
<c:choose>
	<c:when test="${news.commentsCount > 0}">
		<h2 id="comments">
					<spring:message code="viewnews.commentsToNews" />
				</h2>
		<c:forEach var="comment" items="${news.comments}">
			<div class="well well-small">
				<ul class="list-inline">
					<li><small><a
									href="${profileUrl}/${comment.author.id}">
						<strong>${comment.author.login}</strong>
								</a></small></li>
					<li><small><i class="glyphicon glyphicon-time"></i>
						<fmt:formatDate value="${comment.created}" type="both" /></small></li>
				</ul>
	...
	<c:otherwise>
		<h2>
					<spring:message code="viewnews.noComments" />
				</h2>
	</c:otherwise>
</c:choose>
